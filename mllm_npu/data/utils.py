...
def select_best_resolution_v2(original_size, possible_resolutions):
    ...
    for width, height in possible_resolutions:
        ...
        area = width * height
        aspect_ratio_diff = max(aspect_ratio, original_aspect_ratio) / min(
            aspect_ratio, original_aspect_ratio)
        area_ratio = max(area, original_area) / min(area, original_area)

        if aspect_ratio_diff < min_aspect_ratio_diff or (
                aspect_ratio_diff == min_aspect_ratio_diff
...
def resize_and_pad_image(image, target_resolution, keep_ratio=False):
    ...
    if keep_ratio:
        ...
        if scale_w < scale_h:
            ...
            new_height = min(math.ceil(original_height * scale_w),
                             target_height)
        else:
...
def process_anyres_image(image, image_transform, grid_pinpoints,
                         base_image_size):
    """
    ...
    width2, height2 = select_best_resolution_v2(image.size,
    ...
    image_patches = [
        image_transform(image_patch) for image_patch in image_patches
    ]

    patch_grid = (best_resolution[0] // base_image_size,
                  best_resolution[1] // base_image_size)
    x_index = (torch.arange(patch_grid[0]).repeat(patch_grid[1], 1) +
               0.5) / patch_grid[0]
    y_index = (torch.arange(patch_grid[1]).unsqueeze(1).repeat(
        1, patch_grid[0]) + 0.5) / patch_grid[1]
    patch_pos = torch.stack([x_index, y_index], dim=-1).flatten(0, 1)

    origin_pos = torch.tensor([[0.5, 0.5]])
    patch_pos = torch.cat([patch_pos, origin_pos], dim=0)
...
def anyres_data_collate(batch, tokenizer, dataset_name=None):
    ...
    for key in keys:
        ...
        if len(cur) == 0:
            ...
            if key in [
                    'embeds_gen_mask', 'embeds_cmp_mask', 'images',
                    'images_patch_length', 'patch_position', 'image_size'
            ]:
                results[key] = torch.cat(cur, dim=0)
            else:
                if key in ['input_ids']:
                    results[key] = torch.nn.utils.rnn.pad_sequence(
                        cur,
                        batch_first=True,
                        padding_value=tokenizer.pad_token_id)
                elif key in ['attention_mask']:
                    results[key] = torch.nn.utils.rnn.pad_sequence(
                        cur, batch_first=True, padding_value=0)
                elif key in ['labels']:
                    results[key] = torch.nn.utils.rnn.pad_sequence(
                        cur, batch_first=True, padding_value=-100)
                elif key in ['ids_gen_mask', 'ids_cmp_mask']:
                    results[key] = torch.nn.utils.rnn.pad_sequence(
                        cur, batch_first=True, padding_value=False)
