...
def me_attn(query: torch.Tensor,
            key: torch.Tensor,
            value: torch.Tensor,
            attn_bias=None,
            attn_dropout=0.0):
    ...
    attn = query @ torch.transpose(key, -2, -1)
    if attn_bias is not None:
        attn = attn + mc_mask(attn.shape, torch.float16, 'cuda')
    attn = attn.softmax(-1)
    attn = F.dropout(attn, attn_dropout)
    attn = attn @ value
...
class XFormerLlamaAttention(LlamaAttention):
    ...
    #     # support GQA
    #     attn_output = me_attn(
    #         query_states.reshape(bsz, q_len, self.num_key_value_heads, self.num_key_value_groups, self.head_dim),
    #         key_states.reshape(bsz, q_len, self.num_key_value_heads, 1, self.head_dim).expand([bsz, q_len, self.num_key_value_heads, self.num_key_value_groups, self.head_dim]),
    #         value_states.reshape(bsz, q_len, self.num_key_value_heads, 1, self.head_dim).expand([bsz, q_len, self.num_key_value_heads, self.num_key_value_groups, self.head_dim]),
    #         attn_bias=attn_bias,
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        next_cache = None
        if use_cache:
            next_cache = (next_decoder_cache.to_legacy_cache() if isinstance(
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position,
        ...
        if hasattr(getattr(self.layers[0], "self_attn", {}),
                   "past_key_value"):  # static cache
            target_length = self.config.max_position_embeddings
        else:  # dynamic cache
            target_length = (attention_mask.shape[-1] if isinstance(
                attention_mask, torch.Tensor) else current_length + 1)
    ...
    def prepare_inputs_for_generation(self,
        ...
        if past_key_values is None:
            past_key_values = getattr(
                getattr(self.model.layers[0], "self_attn", {}),
                "past_key_value", None)
